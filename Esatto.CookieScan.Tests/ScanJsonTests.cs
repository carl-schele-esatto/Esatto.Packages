...
public class ScanJsonTests
{
    ...
    private static ScanResult Sample() => new(
        ...
        [
            new("_ga_*", "Google Analytics", "statistics", "Mäter.", "24 månader", "Cookie",
                CandidateFlag.NeedsReview, ConsentPass.AcceptAll, "https://example.com/"),
        ],
        Violations:
        [
            new("_fbp", "Meta", "marketing", "Annonser.", "3 månader", "Cookie",
                CandidateFlag.Violation, ConsentPass.RejectAll, "https://example.com/x"),
        ],
        ...
        Outcome: new MergeOutcome(["_ga_*"], ["cookie-consent"], ["old"], PolicyPageKey, true),
        CanReachApi: true,
        DryRun: false,
    ...
    public void A_result_survives_a_round_trip_intact()
    {
        ...
        Assert.NotNull(back);
        Assert.Equal("https://example.com/", back.Site);
