...
public static class ProtectedText
{
    ...
    /// </para>
    /// <para>
    /// Which is exactly why it still reads <c>NDSTK.CookieScanner</c> after this tool was extracted
    /// out of that site and renamed: the string is a namespace, not a name, and every credential
    /// already saved by the pre-extraction build is encrypted under it. Renaming it to match the new
    /// assembly would have cost nothing but correctness - the settings file would still load, and
    /// every member password and client secret in it would come back as "could not be read". Leave
    /// it. It is deliberately the one identity the rename did not touch.
    /// </para>
    /// </remarks>
