...
public sealed record SiteProfile(
    ...
    string ClientId = "",
    // Last of the four credentials, and last of them on purpose: the positional order is the order
    // the file's keys are written in, and appending rather than inserting keeps a hand-read
    // settings.json comparable across builds.
    string ClientSecret = "",
    // Appended after them for that same reason, and not one of them: this is the name of the
...
/// it had a working alternative the other credentials did not - ESATTO_COOKIESCAN_CLIENT_SECRET, set
    ...
    EmailAccount? Email = null)
{
    ...
    /// <c>%LOCALAPPDATA%\Esatto.CookieScan.Engine\settings.json</c> - it is the operator's real list of
