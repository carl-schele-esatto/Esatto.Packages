...
# The NuGet icon for every package in the scanner set, from the same mark, for the same reason: an
# icon.png exported from a design tool would be a binary of unknown origin that nobody could
# regenerate when the mark changed. 128x128 is what nuget.org renders and what the docs recommend.
#
# All four share one mark deliberately. They are one tool split along a dependency boundary - rules,
# engine, command, endpoint - not four products, and a gallery row that showed four different icons
...
foreach ($package in $packages) {
    $folder = Join-Path $repo $package
    if (-not (Test-Path $folder)) {
        Write-Warning "no such package folder: $folder - skipped"
        continue
    }
    $path = Join-Path $folder 'icon.png'
    $icon.Save($path, [System.Drawing.Imaging.ImageFormat]::Png)
    "wrote $path - 128x128, $((Get-Item $path).Length) bytes"
}
$icon.Dispose()
