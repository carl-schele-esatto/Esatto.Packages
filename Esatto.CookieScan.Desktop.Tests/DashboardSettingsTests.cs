...
using Esatto.CookieScan.Desktop;

namespace Esatto.CookieScan.Desktop.Tests;
...
/// <c>%LOCALAPPDATA%\Esatto.CookieScan.Engine</c>, which is the real window's file: a suite that ran
...
public class DashboardSettingsTests
{
    ...
    private sealed class TempSettings : IDisposable
    {
        ...
        private readonly string folder = System.IO.Path.Combine(
            System.IO.Path.GetTempPath(), "Esatto.CookieScan.Desktop.Tests", Guid.NewGuid().ToString("n"));
    ...
    private static SiteProfile Profile(string url) => new(
        ...
        ClientId: "cookie-scanner-test",
        ClientSecret: "client-secret-alpha-1",
        ConsentCookie: "site-consent");
    ...
    public void A_flat_pre_profiles_file_becomes_one_profile()
    {
        ...
        Assert.Equal("old@example.com", profile.MemberEmail);
    ...
    public void Two_profiles_round_trip_through_save_and_load()
    {
        ...
        DashboardSettings saved = new(
            [Profile("https://localhost:44351"), Profile("https://example.com") with { MaxPages = 40 }],
        ...
        Assert.Equal("https://example.com", loaded.SelectedUrl);
    ...
    public void The_saved_file_holds_no_plaintext_credential()
    {
        ...
        // Nor is the consent cookie's name. It is the name of a cookie the site sets on every
        // visitor's browser, so there is nothing to protect and everything to gain from being able
        // to read it: a wrong value here turns the site's own consent cookie into a reported
        // violation, and that is a mistake someone has to be able to spot in the file.
        Assert.Contains(profile.ConsentCookie, text, StringComparison.Ordinal);
        ...
        Assert.Equal("site-consent", stored["ConsentCookie"]!.GetValue<string>());
    }

    /// <summary>
    /// A settings file from before the field existed opens with an empty consent cookie rather than
    /// failing, exactly as the credential fields do.
    /// </summary>
    /// <remarks>
    /// The whole file loads as no sites at all if one profile cannot be constructed, so a missing key
    /// is not a small bug: it would silently empty an operator's list of sites. Same reasoning as the
    /// <c>ClientSecret</c> default beside it, and worth its own test for the same reason.
    /// </remarks>
    [Fact]
    public void A_file_without_a_consent_cookie_key_loads_with_an_empty_one()
    {
        using TempSettings file = new();
        ...
        Assert.Equal("", profile.ConsentCookie);
        Assert.Equal(12, profile.MaxPages);
    }

    // Stored trimmed like every other string in the profile, so a name pasted with a stray space
    // cannot silently stop matching the cookie the browser actually reports.
    [Fact]
    public void An_upserted_consent_cookie_is_trimmed()
    {
        DashboardSettings settings = new([], null);

        settings.Upsert(Profile("https://example.com") with { ConsentCookie = "  site-consent  " });

        Assert.Equal("site-consent", Assert.Single(settings.Sites).ConsentCookie);
    }
    ...
    public void A_file_from_before_the_secret_loads_with_an_empty_secret()
    {
        ...
        string email = ProtectedText.Protect("old@example.com");
    ...
    public void A_corrupted_blob_clears_its_field_and_warns()
    {
        ...
        SiteProfile intact = Profile("https://example.com");
        SiteProfile damaged = Profile("https://localhost:44351");
    ...
    public void A_stored_value_without_the_prefix_is_refused_and_warned_about()
    {
        ...
        document["Sites"]![0]!["MemberEmail"] = "plain@example.com";
    ...
    {
        SiteProfile kept = Profile("https://example.com");
    ...
    public void Saving_a_site_again_replaces_it_rather_than_duplicating_it()
    {
        ...
        settings.Upsert(Profile("https://example.com"));
