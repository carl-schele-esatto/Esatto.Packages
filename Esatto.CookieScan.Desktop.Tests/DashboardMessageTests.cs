...
public class DashboardMessageTests
{
    ...
    public void A_run_command_parses_with_all_its_options()
    {
        const string json = """
            {"type":"run","url":"https://localhost:44351","maxPages":7,"locale":"En",
             ...
             "clientSecret":"client-secret-alpha-1","dryRun":false,"consentCookie":"site-consent"}
            """;
        ...
        Assert.Equal("site-consent", run.ConsentCookie);
    }

    /// <summary>
    /// A run from a page that sends no consent cookie still parses, and means "the default".
    /// </summary>
    /// <remarks>
    /// The field is optional in the field as well as in the record: almost no site renames its
    /// consent cookie. A message missing it has to arrive as a run rather than be dropped by the
    /// loop, or an operator who never touched the field would get a window whose Run button silently
    /// does nothing.
    /// </remarks>
    [Fact]
    public void A_run_command_without_a_consent_cookie_parses_with_none()
    {
        const string json = """
            {"type":"run","url":"https://localhost:44351","maxPages":7,"locale":"En","dryRun":true}
            """;

        RunCommand run = Assert.IsType<RunCommand>(DashboardCommand.Parse(json));

        Assert.Null(run.ConsentCookie);
    }
    ...
    public void A_diff_payload_serialises_under_the_names_the_page_reads()
    {
        string json = JsonSerializer.Serialize(
            new
            {
                recategorised = new[] { new CategoryChange("cookie-consent", "marketing", "necessary") },
        ...
        Assert.Equal("cookie-consent", change.GetProperty("name").GetString());
