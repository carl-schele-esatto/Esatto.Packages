...
/// </summary>
/// <remarks>
/// <c>DeclaredFromCatalogue</c> is the one member worth a note. It holds the declarations sent on
/// the catalogue's word rather than on a sighting - see <c>MergePlanner.UnobservedExpected</c> -
/// and is null both for a history file written before it existed and for a run that had nothing to
/// add from the catalogue; read it as empty either way.
/// <para>
/// It is deliberately NOT folded into <see cref="Candidates"/>, which is the scan's observations and
/// the basis of every comparison between two runs. A catalogue row is present in every run whether
/// the site changed or not, so adding one there would make it appear in a diff as something that
/// happened. Both front ends merge the two lists when they draw their table - that is a rendering
/// decision, and it belongs where the table is built.
/// </para>
/// </remarks>
