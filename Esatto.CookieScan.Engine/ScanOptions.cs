...
/// <para>
/// <paramref name="ConsentCookie"/> is what this site calls the banner's own consent cookie, for a
/// site that changed <c>CookieBannerOptions.CookieName</c> from its <c>cookie-consent</c> default.
/// Null means the default, which is the common case. See
/// <c>CookieCatalogue.WithConsentCookieNamed</c> for what goes wrong without it.
/// </para>
/// </remarks>
...
{
    public const string SecretVariable = "ESATTO_COOKIESCAN_CLIENT_SECRET";
    ...
    public static ScanOptions Parse(string[] args)
    {
        ...
        if (flags.TryGetValue("url", out string? url) is false || string.IsNullOrWhiteSpace(url))
        {
            throw new ArgumentException(
                "--url is required. Example: esatto-cookiescan --url https://example.com");
        }
        ...
        return new ScanOptions(
            ...
            ConsentCookie: Value(flags, "consent-cookie"));
